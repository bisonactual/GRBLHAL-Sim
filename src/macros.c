/*
  ...
  Vendored from grblHAL/Plugin_SD_card for the simulator build.
  Only the #include "sdcard.h" is replaced with "driver.h".
...
static void stack_push (macro_id_t macro_id, uint32_t repeats, vfs_file_t *file, size_t pos, size_t pos_return, bool pstack)
{
    ...
    macro_stack_entry_t *stack = &macro[++stack_idx];
    stack->id = macro_id;
...
{
    char local_tc_path[15];
    if(hal.tool.change == macro_tool_change) {
        strcpy(local_tc_path, path);
        atc_path_fix(local_tc_path);
        if(!strcmp(path, local_tc_path)) {
